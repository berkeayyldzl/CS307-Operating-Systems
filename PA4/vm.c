...
//i couldn't finish writing of the code on time, and i just tried to make the os initialize at first and pass the memory testing, 
//but i couldn't figure out how to fix the memory allocation.
...
int freeMem(uint16_t ptr) { 
    uint16_t *header = mem + ptr - 2;

    if (*(header + 1) != 42) {
        return 0;
    }

    uint16_t *prev = mem + OS_MEM_SIZE;
    while (*(prev + 1) != 0 && *(prev + 1) < header - mem) {
        prev = mem + *(prev + 1);
    }
    ...
    *header += 2;
    if (*(header + *header + 1) == 0) {
        *header += *(header + *header);
    }

    if (*(prev + *prev + 1) == header - mem) {
        *prev += *header;
    }
...
uint16_t allocMem(uint16_t size) { 
    uint16_t *header = mem + OS_MEM_SIZE;
    uint16_t required_size = size + 2;

    while (header != NULL && (*header < required_size || *(header + 1) == 42)) {
        header = mem + *(header + 1);
    }
    ...
    uint16_t alloc_address = header - mem + 2;
    uint16_t remaining_size = *header - required_size;
    *header = size;
    *(header + 1) = 42;

    if (remaining_size > 2) {
        header = header + required_size;
        *header = remaining_size - 2;
        *(header + 1) = 0;
    }
...
uint16_t mr(uint16_t address) {
    //i couldn't finish this on time
}

int createProc(char *fname, char* hname) { 
    //i coulnd't finish this on time
    return 1;
}
